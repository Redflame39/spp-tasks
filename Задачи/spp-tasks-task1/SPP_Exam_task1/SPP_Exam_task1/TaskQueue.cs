...
namespace SPP_Exam_task1
{
    class TaskQueue: IDisposable
    {
        ...
        private readonly object locker = new object(); //locker for memory access control (ограничение доступа к очереди)
        private bool isRun = true; //?queue is running (проверка на запсук потока)


        /**
         * Create new tread poll with fixed treads count
         */
        public TaskQueue(int threadsCount)//конструктор
        {
         ...
         */
        private void QueueConsumeTask()// извлечение и выполнение
             
        {
            while (isRun)
            {
                ...
                lock (locker) //queue access only for 1 process in each moment
                    //конкретный поток блокирует очередь, то тех пор не дождется пульса от другого потока, пока сам не выполниться
                {
